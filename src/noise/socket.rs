...
impl<TSocket> BoundInfo for NoiseSocket<TSocket>
where
    TSocket: BoundInfo,
{
    ...
    TSocket: AsyncWrite + Unpin,
{
    ...
    fn poll_write(&mut self, context: &mut Context<'_>, buf: &[u8]) -> Poll<io::Result<usize>> {
...
{
    /// Perform a Single Round-Trip noise KK handshake returning the underlying [NoiseSocket]
